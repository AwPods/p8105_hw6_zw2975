...
bw = bw |>
  add_predictions(bw, var = "fitted") |>
  add_residuals(bw, var = "residuals")
...
model1 <- lm(bwt ~ blength + gaweeks, data = bw_clean)
model2 <- lm(bwt ~ bhead * blength * babysex, data = bw_clean)
...
```{r}
cv_splits <- crossv_mc(bw_clean, n = 100, test = 0.2)

# Function to compute RMSE for a model
...
cv_results <- cv_splits |>
  mutate(
    model_base = map_dbl(splits, ~ compute_rmse(.x, bwt ~ gaweeks + delwt + smoken + babysex + mrace)),
    model1 = map_dbl(splits, ~ compute_rmse(.x, bwt ~ blength + gaweeks)),
    model2 = map_dbl(splits, ~ compute_rmse(.x, bwt ~ bhead * blength * babysex))
  )
