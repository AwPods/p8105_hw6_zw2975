...
# Regression model
bw_model = lm(bwt ~ gaweeks + delwt + smoken + babysex + mrace, data = bw)
...
# mutate to compare models
cv_df = 
  cv_df |> 
  mutate(
    bw_mod  = map(train, \(df) lm(logratio ~ range, data = bw)),
    m1_mod  = map(train, \(df) gam(logratio ~ s(range), data = df)),
    m2_mod  = map(train, \(df) gam(logratio ~ s(range, k = 30), sp = 10e-6, data = df))) |> 
  mutate(
    rmse_bw = map2_dbl(bw_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_m1 = map2_dbl(m1_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_m2 = map2_dbl(m2_mod, test, \(mod, df) rmse(model = mod, data = df)))

cv_splits = crossv_mc(bw, n = 100, test = 0.2)

# using a function to compute RMSE
compute_rmse = function(split, formula) {
...
cv_results = cv_splits |>
  mutate(
    model_base = map_dbl(split, ~ compute_rmse(.x, bwt ~ gaweeks + delwt + smoken + babysex + mrace)),
    model1 = map_dbl(split, ~ compute_rmse(.x, bwt ~ blength + gaweeks)),
    model2 = map_dbl(split, ~ compute_rmse(.x, bwt ~ bhead * blength * babysex)))

# Summarize RMSE for each model
cv_results_summary = cv_results |>
summarise(
    base_rmse = mean(model_base),
    model1_rmse = mean(model1),
    model2_rmse = mean(model2))
...
#### Comparison
```{r comaprison between models}
# Reshape results for plotting
cv_results_long = cv_results |>
  pivot_longer(cols = starts_with("model"), names_to = "model", values_to = "rmse")
